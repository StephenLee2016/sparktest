...
object Ex3_CombiningRDDs {
  def main(args: Array[String]): Unit ={
    ...
    //substract one from another, getting yet another RDD of the same type
    val consonants = letters.subtract(letters)
    ...
    println("There aren't relly " + tooManyVowels.count() + " vowels")
    ...
    println("This may not be what you expect with unequal length RDDs")
    unequalOK foreach{
      case (c, i) => println(i + ": " + c)
    }
